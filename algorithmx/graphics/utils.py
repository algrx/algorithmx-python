...
def get_attr_entry(sel: SelectionContext, arg, attr, index: int):
    if callable(attr):
        return (
            attr(call_element_fn(arg, sel.data[index], index))
            if callable(arg)
            else attr(arg)
        )
    else:
...
def create_parent_attr(sel: SelectionContext, arg, attr):
    if callable(attr) and sel.data is None:
        ...
        return {
            sel.name: {
                k: get_attr_entry(sel, arg, attr, i) for i, k in enumerate(sel.ids)
            }
        }
...
def attr_event(sel: SelectionContext, arg, attr) -> Dict:
    ...
    return {
        "type": DispatchEventType.Highlight
        if sel.highlight
        else DispatchEventType.Update,
        "queue": sel.queue,
...
def queue_event(
    sel: SelectionContext, event_type: str, queue: Union[Any, None]
) -> Dict:
    queues = None if queue is None else [str(queue)]
    return {"type": event_type, "queue": sel.queue, "data": {"queues": queues}}
...
def update_animation(sel: SelectionContext, arg, attr) -> Dict:
    if (
        len(sel.animation) == 0 or (len(sel.animation) == 1 and "**" in sel.animation)
